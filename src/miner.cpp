...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("TeslaMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("TeslaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadTeslaMiner(void* parg);
...
void TeslaMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        printf("Running TeslaMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadTeslaMiner(void* parg)
{
    ...
    try
    {
        ...
        TeslaMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadTeslaMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadTeslaMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadTeslaMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d TeslaMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadTeslaMiner, pwallet))
                printf("Error: NewThread(ThreadTeslaMiner) failed\n");
            Sleep(10);
        }
    }
}
